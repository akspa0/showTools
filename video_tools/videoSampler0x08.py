...
def sanitize_filename(filename: str) -> str:
    ...
    # Remove any double underscores and strip
...
def safe_file_operation(operation, src, dst, error_msg="File operation failed"):
    """
    ...
    try:
        ...
        print(f"Error: Permission denied - {dst}")
    ...
    def process_video(self, video_path: str) -> List[FrameScore]:
        ...
        with tqdm(total=self.frames_per_video, desc=f"Processing {os.path.basename(video_path)}") as pbar:
            ...
            while len(frame_scores) < self.frames_per_video and frame_count < total_frames:
                ...
                base_filename = sanitize_filename(os.path.basename(video_path))
                frame_filename = f"{base_filename}_{frame_count:08d}.jpg"
                ...
                if len(frames_batch) >= 8 or frame_count + interval >= total_frames:
                    for i, (frame, timestamp, frame_path) in enumerate(zip(frames_batch, timestamps_batch, paths_batch)):
                        try:
                            ...
                            if os.path.exists(frame_path):  # Verify the file was saved
    ...
    def organize_frames_by_year(self, scores: List[FrameScore]):
        ...
        for year, year_scores in years_dict.items():
            ...
            for score in year_scores:
                if not os.path.exists(score.frame_path):
                    print(f"Warning: Source file not found - {score.frame_path}")
                    continue
                
                try:
                    # Move to year directory
                    new_base_path = os.path.join(base_year_dir, sanitize_filename(os.path.basename(score.frame_path)))
                    safe_file_operation(shutil.move, score.frame_path, new_base_path)
                    
                    if os.path.exists(new_base_path):  # Verify the move was successful
                        score.frame_path = new_base_path
                        
                        # Copy to aesthetic directory if score is high enough
                        if score.overall_score >= self.aesthetic_threshold:
                            aesthetic_filename = f"score_{score.overall_score:.3f}_{sanitize_filename(os.path.basename(score.frame_path))}"
                            aesthetic_path = os.path.join(aesthetic_year_dir, aesthetic_filename)
                            safe_file_operation(shutil.copy2, new_base_path, aesthetic_path)
                    
                except Exception as e:
                    print(f"Error organizing frame: {str(e)}")
                    print(f"Source: {score.frame_path}")
                    print(f"Destination: {new_base_path}")
