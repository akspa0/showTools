...
# Function to merge valid audio files in each subfolder
def merge_audio_in_subfolders(input_folder, output_folder):
    # Walk through each subfolder in the input directory
    for root, _, files in os.walk(input_folder):
        valid_files = []
        
        # Identify all valid audio files based on the prefix rule
        for filename in sorted(files):
            if is_valid_audio(filename) and filename.endswith(('.wav', '.mp3', '.ogg', '.flac')):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Merge audio files with a 1-9999 prefix from subfolders into merged files.")
    ...
    merge_audio_in_subfolders(args.input_folder, args.output_folder)
